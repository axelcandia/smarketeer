...
class API extends \Piwik\Plugin\API
{
   ...
   /**
    ...
    public function getSourcesById($userId,$idSite){
        Db::get();
        $query="SELECT campaign_source,referer_name,referer_url,visit_last_action_time as lastActionDateTime
             FROM bitnami_piwik.piwik_log_visit 
             WHERE idSite=$idSite
             AND user_id='$userId'
             ORDER BY visit_last_action_time DESC
            ";
    ...
    public function GetAllSources($idSite,$idGoal,$from,$to){
        Db::get();
        $query="SELECT ref as campaign_source, sum(total) as total from (
                                SELECT (SUBSTRING_INDEX(SUBSTRING_INDEX((SUBSTRING_INDEX((SUBSTRING_INDEX(campaign_source, '://', -1)), '/', 1)), '.', -2),'.',1)) as ref
                ,COUNT(campaign_source) AS total
                FROM bitnami_piwik.piwik_log_conversion 
                WHERE campaign_source IS NOT NULL 
                AND idSite=$idSite
                AND idgoal=$idGoal
                AND server_time BETWEEN DATE_ADD('$from', INTERVAL 1 DAY) and DATE_ADD('$to', INTERVAL 1 DAY)
                GROUP BY ref


                UNION ALL

                                SELECT (SUBSTRING_INDEX(SUBSTRING_INDEX((SUBSTRING_INDEX((SUBSTRING_INDEX(referer_name, '://', -1)), '/', 1)), '.', -2),'.',1)) as ref
                ,COUNT(idvisitor) AS total
                FROM piwik_log_conversion 
                WHERE idSite=10
                AND idgoal=$idGoal
                AND campaign_source is null
                AND server_time BETWEEN DATE_ADD('$from', INTERVAL 1 DAY) and DATE_ADD('$to', INTERVAL 1 DAY)
